...
class MixATSPModel(nn.Module):

    def __init__(self, **model_params):
        super().__init__()
        self.model_params = model_params

        self.encoder = MixEncoder(**model_params)
    ...
    def forward(self, state):
        ...
        if state.current_node is None:
            ...
            job_log_p = torch.zeros(size=(batch_size, pomo_size))
            ...
            if self.training or self.model_params['eval_type'] == 'softmax':
                ...
                job_log_p = dist.log_prob(selected)
            else:
                selected = all_job_probs.argmax(dim=2)
                # shape: (batch, pomo)
                job_log_p = torch.zeros(size=(batch_size, pomo_size))

        return selected, job_log_p
